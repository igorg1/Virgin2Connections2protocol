...
//**********************************************************************************
void ErrorResponseMB(ProtInstanse* mb, uint8_t ucFunctionCode, eMBException eException)
{
...
//**********************************************************************************
void FrameProcess_Modbus(ProtInstanse* pr)
{
	uint16_t qty = 0;
	uint16_t rcv_crc = 0;
...
//	NRF_LOG_HEXDUMP_DEBUG(pr->mBuf, pr->mLen);
	...
	if(MB_RTU_PDU_SIZE_MIN > pr->mLen
		|| 0 != rcv_crc )
//		|| rcv_address != pr->addr )
	{
		pr->state = NotInit;
	...
	uint16_t pusLength = (uint16_t)( pr->mLen - MB_RTU_PDU_PDU_OFF - MB_RTU_PDU_SIZE_CRC );
	...
	if(rcv_address == pr->addr)
	{
		...
		case MB_FUNC_WRITE_MULTIPLE_REGISTERS:
			eException = eMBFuncWriteMultipleHoldingRegister(pucFrame, &pusLength);
			break;
		}
...
//**********************************************************************************
void OnTxCompleate_Modbus(void* prot)
{
