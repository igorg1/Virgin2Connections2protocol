...
void BLCfg_Load(void) {
  ...
  const BL_Storage *bl_stor = (const BL_Storage *)Flash_LoadStor(FLASH_BLCFG_START_ADDR, FLASH_BLCFG_END_ADDR, sizeof(BL_Storage));
...
void APPCfg_Load(void) {
  ...
  const APP_Storage *app_stor = (const APP_Storage *)Flash_LoadStor(FLASH_APPCFG_START_ADDR, FLASH_APPCFG_END_ADDR, sizeof(APP_Storage));
...
void APPCfg_Save(void) {
  ...
  const uint32_t bl_pass_CRC = crc32_compute((uint8_t const *)&sBL_LockCtrl.mPasswd, psz, NULL);
...
void LockCtrl_Configurate() {
  ...
  if (sLockCfg.mLock) {
    ...
    {
      const uint32_t psz = sizeof(sLockCtrl.mPasswd);
      gCurrentCRC = sLockCfg.mPasswdCRC = crc32_compute((uint8_t const *)&sLockCtrl.mPasswd, psz, NULL);
    } else
      sLockCtrl.mLockCmd = sLockCfg.mLock;
  }
  sSysCtrl.mStatus++;
